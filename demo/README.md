...
a typed action proposal, and a deterministic approval stop. They contain synthetic names and
...
`ActionRequest`, and uses the real deterministic `ActionPolicyEngine` to assess risk. It never
invokes an executor. Every included scenario therefore ends at `WAITING_APPROVAL`.
